...
<div class="card bg-base-300 my-2 flex flex-col p-4">
	<div class="flex w-full flex-row justify-between">
		...
		<p class="badge badge-secondary badge-xl"><Fa icon={faClock}/>{(new Date(currInfo.startTime))
				.getHours()
				.toString()
				.padStart(2, '0')}:{(new Date(currInfo.startTime)).getMinutes().toString().padStart(2, '0')}</p>
